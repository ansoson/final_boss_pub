...
public abstract class FXFrontEnd extends CS1971FrontEnd {
	...
	protected final void init(Stage stage) throws IOException, ParserConfigurationException, TransformerException, InvocationTargetException, SAXException, NoSuchMethodException, InstantiationException, IllegalAccessException {
		this.stage = stage;
		...
		if(running) {
			...
			timer.getKeyFrames().add(new KeyFrame(Duration.millis(tickDelay), e -> {
				try {
					...
					throw new RuntimeException(ex);
				} catch (IllegalAccessException | IOException | TransformerException ex) {
					throw new RuntimeException(ex);
				}
			}));
			timer.play();
	...
	private final void callAllTick() throws InvocationTargetException, NoSuchMethodException, InstantiationException, IllegalAccessException, IOException, TransformerException {
