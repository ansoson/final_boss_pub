...
public class RoundedRectangle extends UIObject{

    public RoundedRectangle(Vec2d pos, Vec2d size, Vec2d def, Color c){
        super(c, pos, size, def);

        this.c=c;
    }

    public RoundedRectangle(UIObject parent, Vec2d pos, Vec2d size, Vec2d def, Color c){
    ...
    public void DrawElement(GraphicsContext g) {
        if (active){
            ...
            g.setFill(c);
            g.setLineWidth(1);
            g.fillRoundRect(pos.x, pos.y, size.x, size.y, size.x/4, size.y/4);
