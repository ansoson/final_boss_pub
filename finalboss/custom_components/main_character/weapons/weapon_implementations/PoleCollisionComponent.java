...
public class PoleCollisionComponent extends CollisionComponent {
    ...
    public PoleCollisionComponent(GameObject g, ShapeEnum s, int layer, boolean stat, Direction dir, double range) {
        ...
        if (debug) {
            ...
            if (current < 19 && current > 16) {
                if(dir == Direction.LEFT) {
                    this.shape = new ColAAB(parent.getTransform().pos.plus((float) (-260*range),9), new Vec2d(260*range,32), parent.getTransform().def);
                    a = new Rectangle(converter(parent.getTransform().pos.plus((float) (-260*range), 9)), new Vec2d(260*range, 32), parent.getTransform().def, Color.RED);
                }
                else{
                    this.shape = new ColAAB(parent.getTransform().pos.plus(10,9), new Vec2d(260*range,32), parent.getTransform().def);
                    a = new Rectangle(converter(parent.getTransform().pos.plus(10, 9)), new Vec2d(260*range, 32), parent.getTransform().def, Color.RED);
                }
            }
            else{
